...
*During the fault injection tests, I noticed that the system didn’t completely stop when the worker was disabled. The API continued accepting votes and RabbitMQ kept buffering messages, but Supabase stopped updating, which was confusing at first because there were no clear errors. Seeing the queue continuously grow helped me understand how distributed systems handle partial failures by isolating the problem instead of shutting everything down. It made me realize how important message queues are in preventing data loss during downtime. When the worker was restored, I expected it to immediately process the queued messages, but like what others experienced, it sometimes required a redeploy before it fully reconnected. Once it was working again, it processed all the accumulated votes and Supabase updated in batches, which clearly showed how eventual consistency works. This made me see the trade-off between simplicity and reliability—while the system is more fault-tolerant and scalable, it is also harder to debug and less predictable compared to a simple sequential program.*
