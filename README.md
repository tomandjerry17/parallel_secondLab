...
*When the worker was disabled, the API still accepted votes and Pub/Sub continued buffering messages, which was confusing at first since Firestore was not updating. However, seeing that no data was lost helped me understand how fault tolerance works and how message queues protect the system during failures. When the worker was restored, it automatically processed all the queued votes, which showed how recovery and eventual consistency work in practice. At the same time, I noticed that while the system is more reliable and scalable, it is also more complex and harder to debug compared to a simple setup.*
